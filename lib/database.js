...
function save_tx_prefetched(txid, block, cb) {
  lib.get_rtx_cached(txid, function(tx){
    if (tx != 'There was an error. Check your console.') {
          ...
          lib.syncLoop(vin.length, function (loop) {
            ...
            lib.syncLoop(vout.length, function (subloop) {
              ...
              lib.calculate_total(vout, function(total){
                ...
                console.log('Commit tx: (%s)', tx.txid);
...
module.exports = {
  ...
  create_txs: function(block, cb) {
    lib.syncLoop(block.tx.length, function (loop) {
      var i = loop.iteration();
      save_tx_prefetched(block.tx[i], block, function(err){
        if (err) {
          loop.next();
        } else {
  ...
  get_last_txs: function(count, min, cb) {
    Tx.find({'total': {$gt: min}}).sort({"blockindex": 'desc'}).limit(count).exec(function(err, txs){
      if (err) {
        return cb(err);
      } else {
        return cb(txs);
  ...
  get_last_blocks: function (limit, cb) {
    var safe_limit = limit || 5;
    Tx.aggregate( [
        { $group: { '_id': '$blockindex', 'blockhash': { $first: "$blockhash"}, 'timestamp': { $first: "$timestamp"} } },
        { $sort: { '_id': -1 } },
        { $limit: safe_limit },
    ] ).sort({"_id": "desc"}).exec(function(err, txs){
      if (err) {
        return cb(err);
      } else {
        return cb(txs);
            ...
            if (block) {
              lib.syncLoop(block.tx.length, function (subloop) {
                ...
                Tx.findOne({txid: block.tx[i]}, function(err, tx) {
                  if(tx) {
                    console.log('Tx cache hit: (%s)', block.tx[i]);
                    tx = null;
                    subloop.next();
                  } else {
                    save_tx_prefetched(block.tx[i], block, function(err){
